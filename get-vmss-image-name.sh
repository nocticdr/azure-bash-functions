...
# vmssimage <resource-group> <agw-name>

vmssimage() 
{
  rg=$1
  name=$2
  image=`az vmss show -n $name -g $rg -o json | jq -r .virtualMachineProfile.storageProfile.imageReference.id | sed 's/^.*images\///'`
  echo "Current image for ${CYAN}$name ${NC}is ${GREEN}$image"
}
